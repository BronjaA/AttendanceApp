...
class UrediVezbe extends Component {
  constructor(props){
    super(props);
    this.state = {
      jwt: '',
  ...
  registrujPredavanje(){
    ...
    .then((response) => {
      Alert.alert('Uspeh!', 'Uspešno ste izmenili vežbe');
  ...
  kreiraj12Predavanja(){
    ...
    // var dayy = pom.setTime(pom.getTime() + (7 * 24 * 60 * 60 * 1000));
    // var dayyy = new Date(dayy);
    ...
    return (
        <View style={styles.container}>
          <TouchableOpacity onPress={() => console.log(this.sviDatumi)}>
            <Text style={{fontSize: 19, marginTop: 20, marginLeft: 20, marginBottom: 7, color: '#009387'}}>Uredi vežbe </Text>
          </TouchableOpacity>
          ...
          <SubjectBtn onPress={() => {
            ...
            else
            {
              ...
              }} style={{marginTop: 100, borderWidth: 2, borderColor: '#009387', padding: 10, marginHorizontal: 20}}>Izmeni vežbe</SubjectBtn>
          }
