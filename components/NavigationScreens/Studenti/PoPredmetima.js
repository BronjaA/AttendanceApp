...
class PoPredmetima extends Component {
  constructor(props){
    super(props);
    this.state = {
      jwt: '',
  ...
  listajPredmete(){
    ...
    .then((response) => {
      ...
      {
        this.predmeti.push({_id: 'x', empty: true})
        totalLastRow ++;
      }
    })
    .catch((error) => {
...
drawDate(datum){
  ...
  _renderItem = ({item, index}) => {
    ...
    return (
      <TouchableOpacity style={[styles.item, {flexDirection: 'row'}]} onPress={() => this.props.navigation.navigate('Studenti', {predmet: item})}>
